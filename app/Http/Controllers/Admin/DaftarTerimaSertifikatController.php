...
{
public function exportByKelas($id)
{
    $kelas = request('kelas');

    if (!$kelas) {
        abort(404, 'Parameter kelas tidak ditemukan.');
    }

    $jadwal = JadwalPelatihan::with(['pendaftars.user.biodata', 'kabupatenkota', 'provinsi'])->findOrFail($id);

    // Filter peserta berdasarkan kelas dan sudah disetujui
    $pesertas = $jadwal->pendaftars->filter(function ($p) use ($kelas) {
        return $p->user->biodata?->is_approved && $p->kelas === $kelas;
    });

    if ($pesertas->isEmpty()) {
        abort(404, 'Tidak ada peserta yang disetujui di kelas ini.');
    }
    ...
    $pdf->MultiCell(0, 8, strtoupper($jadwal->judul . ' - KELAS ' . strtoupper($kelas)), 0, 'C');

    $kab = strtoupper($jadwal->kabupatenkota->nama ?? '-');
    $prov = strtoupper($jadwal->provinsi->nama ?? '-');
    $labelWilayah = \Illuminate\Support\Str::startsWith(strtolower($kab), 'kota') ? $kab : 'KABUPATEN ' . $kab;
    ...
    return response()->streamDownload(function () use ($pdf) {
        $pdf->Output();
    }, 'daftar-terima-sertifikat-kelas-' . strtolower($kelas) . '.pdf');
}
    ...
    {
        $jadwal = JadwalPelatihan::with(['pendaftars.user.biodata', 'kabupatenkota', 'provinsi'])->findOrFail($id);
        $pesertas = $jadwal->pendaftars->filter(fn($p) => $p->user->biodata?->is_approved);
