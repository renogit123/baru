...
<div class="max-w-4xl mx-auto p-6 bg-white rounded shadow">
    ...
    <form method="POST" action="{{ route('user.biodata.store') }}">
        ...
        <x-text-input name="nama" id="nama" type="text" class="block w-full mb-4" value="{{ old('nama', $biodata->nama ?? '') }}" />
    ...
    <x-input-error :messages="$errors->get('id_desa')" class="mt-2" />
        ...
        <x-text-input name="nik" id="nik" type="text" class="block w-full mb-4" value="{{ old('nik', $biodata->nik ?? '') }}" />

        {{-- NPWP (opsional) --}}
        <x-input-label for="npwp" value="NPWP (opsional)" />
        <x-text-input name="npwp" id="npwp" type="text" class="block w-full mb-4" value="{{ old('npwp', $biodata->npwp ?? '') }}" />

        {{-- Tempat Lahir --}}
        <x-input-label for="tempat_lahir" value="Tempat Lahir" />
        <x-text-input name="tempat_lahir" id="tempat_lahir" type="text" class="block w-full mb-4" value="{{ old('tempat_lahir', $biodata->tempat_lahir ?? '') }}" />
        ...
        <x-text-input name="jabatan" id="jabatan" type="text" class="block w-full mb-4" value="{{ old('jabatan', $biodata->jabatan ?? '') }}" />

        {{-- Lama Menjabat --}}
        <x-input-label for="lama_menjabat" value="Lama Menjabat" />
        <x-text-input name="lama_menjabat" id="lama_menjabat" type="text" class="block w-full mb-4" value="{{ old('lama_menjabat', $biodata->lama_menjabat ?? '') }}" />

        {{-- Nomor SK Jabatan --}}
        <x-input-label for="nomor_sk_jabatan" value="Nomor SK Jabatan" />
        <x-text-input name="nomor_sk_jabatan" id="nomor_sk_jabatan" type="text" class="block w-full mb-4" value="{{ old('nomor_sk_jabatan', $biodata->nomor_sk_jabatan ?? '') }}" />
        ...
        {{-- No Telepon --}}
        <x-input-label for="no_telp" value="No Telepon Peserta" />
        <x-text-input name="no_telp" id="no_telp" type="text" class="block w-full mb-4" value="{{ old('no_telp', $biodata->no_telp ?? '') }}" />
