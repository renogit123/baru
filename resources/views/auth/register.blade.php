...
<x-text-input id="name" name="name" type="text" value="{{ old('name') }}"
    class="w-full mb-1 bg-white/90 text-gray-900" autofocus />
@error('name')
    <p class="text-sm text-red-400 mt-1">{{ $message }}</p>
@enderror

<x-input-label for="email" value="Email" class="text-white mt-4" />
...
<x-input-label for="password" value="Kata Sandi" class="text-white mt-4" />
...
<x-input-label for="password_confirmation" value="Konfirmasi Kata Sandi" class="text-white mt-4" />
<x-text-input id="password_confirmation" name="password_confirmation" type="password"
    ...
    <x-text-input name="nama" id="nama" class="block w-full mb-1 bg-sky-900 text-white" value="{{ old('nama') }}" />
    @error('nama')
        <p class="text-sm text-red-400 mt-1">{{ $message }}</p>
    @enderror

    <x-input-label for="alamat" value="Alamat" class="text-white mt-4" />
    <textarea name="alamat" id="alamat" class="block w-full mb-1 bg-sky-900 text-white rounded">{{ old('alamat') }}</textarea>
    @error('alamat')
        <p class="text-sm text-red-400 mt-1">{{ $message }}</p>
    @enderror
    ...
    @foreach (['provinsi', 'kabupaten', 'kecamatan', 'desa', 'kode_desa'] as $field)
        <div class="mt-2">
            <label class="block font-semibold text-white">{{ ucwords(str_replace('_', ' ', $field)) }}</label>
    ...
    @foreach($fields as $key => $label)
        <x-input-label for="{{ $key }}" value="{{ $label }}" class="text-white mt-4" />
        <x-text-input id="{{ $key }}" name="{{ $key }}"
            type="{{ $key === 'tanggal_lahir' ? 'date' : ($key === 'lama_menjabat' ? 'number' : 'text') }}"
    ...
    @foreach ($dropdowns as $name => $options)
        <x-input-label for="{{ $name }}" value="{{ ucwords(str_replace('_', ' ', $name)) }}" class="text-white mt-4" />
        <select id="{{ $name }}" name="{{ $name }}"
            class="bg-sky-900 border border-yellow-400/30 rounded-lg w-full text-white mb-1">
