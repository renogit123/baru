<x-admin-layout>
    <h2 class="text-xl font-semibold text-white">Manajemen Wilayah - Kabupaten/Kota</h2>

    <div class="p-6 space-y-10 bg-gradient-to-br from-[#0a1f44] to-[#1e40af] min-h-screen text-white">
        {{-- ========== FORM TAMBAH / EDIT ========== --}}
        @php $editItem = $editKabupaten ?? null; @endphp
        <div class="bg-white/5 p-6 rounded-lg shadow border border-white/10 backdrop-blur">
            <h3 class="text-lg font-semibold mb-4 text-white">Kabupaten / Kota</h3>
            <form method="POST" action="{{ isset($editItem) ? route('admin.kabupaten-kota.update', $editItem->id) : route('admin.kabupaten-kota.store') }}">
                @csrf
                @if(isset($editItem)) @method('PUT') @endif
                <div class="grid md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-white/90">Pilih Provinsi</label>
                        <select name="provinsi_id" class="mt-1 block w-full border border-gray-300 rounded-md text-black">
                            <option value="">-- Pilih Provinsi --</option>
                            @foreach ($provinsis as $provinsi)
                                <option value="{{ $provinsi->id }}" {{ old('provinsi_id', $editItem->provinsi_id ?? '') == $provinsi->id ? 'selected' : '' }}>
                                    {{ $provinsi->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-white/90">Nama Kabupaten/Kota</label>
                ...
                <div class="mt-4">
                    ...
                    @if(isset($editItem))
                        <a href="{{ route('admin.kabupaten-kota.index') }}" class="ml-3 text-sm text-white/70 hover:underline">Batal</a>
                    @endif
                </div>
            </form>
        </div>
        ...
        <div
            x-data="{
                show: JSON.parse(localStorage.getItem('edit_kabupaten_kota_show') ?? 'false'),
                toggle() {
                    this.show = !this.show;
                    localStorage.setItem('edit_kabupaten_kota_show', JSON.stringify(this.show));
                }
            }"
            class="bg-white/5 p-6 rounded-lg shadow border border-white/10 backdrop-blur"
        >
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-semibold text-white">Daftar Kabupaten/Kota</h3>
            ...
            <div x-show="show" x-transition>
                <form method="GET" class="mb-4">
                    <input type="text" name="search_kabupaten_kota" value="{{ request('search_kabupaten_kota') }}" placeholder="Cari..." class="border px-3 py-1 rounded w-64 text-black">
                        ...
                        <tbody class="divide-y divide-white/10">
                            @foreach($kabupatens as $item)
                                <tr>
                                    ...
                                    <td class="px-4 py-2 text-sm text-white/90">{{ $item->provinsi->nama ?? '-' }}</td>
                                    <td class="px-4 py-2 text-sm space-x-2">
                                        <a href="{{ route('admin.kabupaten-kota.index', ['edit_kabupaten_kota' => $item->id]) }}" class="px-3 py-1 bg-yellow-300 text-xs rounded hover:bg-yellow-400 text-blue-900">✏️ Edit</a>
                                        <form method="POST" action="{{ route('admin.kabupaten-kota.destroy', $item->id) }}" class="inline-block" onsubmit="return confirm('Hapus data ini?')">
                                            @csrf @method('DELETE')
